...
def define_objective_function(garden, assignment):
    ...
    a = .005
    sum(assignment[(group, slot)] for group in garden.availability_groups for slot in time_slots)
    + a * sum(
        assignment[(group, slot)] *
        (
            10 if slot == garden.availability_groups[group][0] else
            5 if len(garden.availability_groups[group]) > 1 and slot == garden.availability_groups[group][1] else
            2 if len(garden.availability_groups[group]) > 2 and slot == garden.availability_groups[group][2] else
            0
        )
