...
def format_output(garden: Garden, solver: cp_model.CpSolver):
    ...
    output_data = {
        "Tuinlocatie": [tuinlocatie] + [""] * (len(time_slots) - 1),
        "Tijdslot": time_slots,
    }
    ...
    for group in unassigned_groups:
        ...
        new_row = pd.DataFrame(
            [[group, schoolname, transportation]],
...
def get_summary_statistics(
    ...
    if solved:
        ...
        # We make a table with the unassigned groups and their preferences
        # There first option will get a 1, the second a 2, etc.
        unassigned_groups = list(unassigned_groups)
        unassigned_groups_preference = pd.DataFrame(
            index=unassigned_groups, columns=garden.time_slots
        )
        unassigned_groups_preference = unassigned_groups_preference.fillna("")
        for group in unassigned_groups:
            group_preference = group_availability[group]
            for i, time_slot in enumerate(group_preference):
                if time_slot in garden.time_slots:
                    unassigned_groups_preference.at[group, time_slot] = i + 1

        # Set the teachers as index for the current educator data to make it pretty in the front-end
        current_educator_data.set_index("educator", inplace=True)
        ...
        summary = {
            ...
            "unassigned_groups_preference": unassigned_groups_preference,
        }

    else:
        # TODO: change this text to being in Dutch!
        summary = (
            "Met de huidige data en de huidige constraints kon het rooster probleem niet worden opgelost. "
            "Controleer de data en constraints en probeer opnieuw."
        )
...
def combine_garden_stats(all_summary_stats: dict) -> dict:
    ...
    for garden in all_gardens:
        ...
        combined_stats["unassigned_groups_preference"] = pd.concat(
            [
                combined_stats["unassigned_groups_preference"],
                all_summary_stats[garden]["unassigned_groups_preference"],
            ],
            axis=0,
        )
